...
import { Sparkles, Upload, Loader2, Image } from "lucide-react";
import type { PhotoPreview } from "../../editor/photo-preview-dialog";
...
export default function PhotoPreviewGrid({
  ...
  const hasUnprocessed = photos.some((p) => !p.isBgRemoved && !p.isProcessing);
    ...
    >
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {hasUnprocessed && (
          <Button
            ...
            disabled={isModelLoading || processingCount > 0}
            variant="outline"
            className="bg-primary hover:bg-secondary/10 hover:border-primary cursor-pointer text-white"
          >
            {isModelLoading ? (
              <>
      ...
      {photos.length === 0 && (
        ...
        >
          ...
          <p className="text-muted-foreground">No photos to preview</p>
        </motion.div>
      )}
        ...
        >
          ...
          {onCreateStickerPack && (
            <Button
              ...
              disabled={
                isUploading || photos.length === 0 || processingCount > 0
              }
